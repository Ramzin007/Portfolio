...
export function Testimonials() {
  return (
    <Section id="testimonials" eyebrow="Testimonials" title="Clients and collaborators notice the finish.">
          ...
          {[...testimonials, ...testimonials].map((testimonial, index) => (
            <Card key={`${testimonial.name}-${index}`} className="w-[20rem] p-6 sm:w-[28rem]">
              <div className="flex gap-1 text-amber-400">
                {Array.from({ length: testimonial.rating }).map((_, star) => <Star key={star} className="size-4 fill-current" />)}
              </div>
